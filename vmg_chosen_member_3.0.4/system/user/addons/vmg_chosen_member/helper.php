...
class ChosenHelper
{
    ...
    public function __construct()
    {
        ...
        $this->allMemberFields = $this->all_users();
    }
    ...
    {
        if(VMG_JS_TYPE == "select2")
        {
            return array(
                ee()->config->item('theme_folder_url') . 'user/vmg_chosen_member/select2/select2.min.css',
            );
        }
        else
        {
    ...
    public function buildJs()
    {
        ...
        {
            return array(
                ee()->config->item('theme_folder_url') . 'user/vmg_chosen_member/select2/select2.min.js',
                ee()->config->item('theme_folder_url') . 'user/vmg_chosen_member/select2/select2-config.js',
            );
        }
        else
        {
    ...
    function all_users()
    {
        ...
        if($get->num_rows > 0)
        {
            $tempResults            = $get->result_array();
            $data['dynamic_fields'] = array();

            foreach ($tempResults as $value)
            {
                $data['dynamic_fields'][$value['m_field_name']] = array(
                    'id'        => $value['m_field_id'],
                    'name'      => $value['m_field_name'],
                    'select'    => "md.m_field_id_" . $value['m_field_id'] . " AS \"" . $value['m_field_name'] ."\"",
                );
            }
            unset($tempResults);
            ...
            if ( defined('VMG_TAG_TYPE'))
            {
                ...
                preg_match_all('#\{(.*?)\}#m', VMG_TAG_TYPE, $temp[]);
                preg_match_all('#\{(.*?)\}#m', VMG_OPTION_TYPE, $temp[]);
            }

            $customData = array();
            for ($i = 0; $i < count($temp); $i++)
            {
                foreach ($temp[$i][1] as $value)
                {
